#!/usr/bin/env bash
# ============================================================================
#  Qadhaaif al-Haqq Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/42-qadhaaif-al-haqq}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.qadhaaifalhaqq}"
AVD_NAME="${AVD_NAME:-qadhaaif_al_haqq_test}"
...
cmd_test() {
  ...
  local out="/tmp/qadhaaif-al-haqq-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   Qadhaaif al-Haqq Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
