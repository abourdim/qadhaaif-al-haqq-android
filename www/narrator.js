/* ═══════════════════════════════════════════════════════════
   NARRATOR.JS — Audiobook narrator for Qadhaaif al-Haqq
...
(function() {
  ...
  function extractCards(panel) {
    ...
    if (tabName === 'about') {
      ...
      panel.querySelectorAll('.truth-card').forEach(el => {
        ...
        const t = el.querySelector('.truth-title');
        const m = el.querySelector('.myth-side .mr-text');
        const r = el.querySelector('.reality-side .mr-text');
        ...
        cards.push({ el, text: cleanText(text), type: 'truth' });
      });
    } else if (tabName === 'myths') {
      panel.querySelectorAll('.myth-flip-card').forEach(el => {
        const text = cleanText(el.textContent);
        cards.push({ el, text, type: 'myth' });
      });
  ...
  function cleanText(text) {
    ...
    if (l === 'fr') {
      clean = clean
        ...
        .replace(/Faisal/gi, 'Faycal').replace(/Qadhaaif/gi, 'Kadaif')
  ...
  function readCurrentCard() {
    ...
    if (card.type === 'truth' && card.el && !card.el.classList.contains('open')) { card.el.classList.add('open'); }
  ...
  function readDuoTranslation(card, onEnd) {
    ...
    const truthId = card.el ? card.el.id : '';
    const tMatch = truthId.match(/truth-(\d+)/);
    if (tMatch && typeof TRUTHS !== 'undefined') {
      const t = TRUTHS.find(tr => tr.id === parseInt(tMatch[1]));
      if (t && t[duoLang]) { duoText = t[duoLang].title + '. ' + t[duoLang].reality; }
    }
  ...
  function switchToTab(tabName) {
    ...
    if (tabBtn) {
      ...
      setTimeout(function() {
        ...
        if (title) {
          speak(title.textContent, function() { if (!STATE.playing) return; setTimeout(function() { if (STATE.playing) readCurrentCard(); }, 300); });
  ...
  function setupMediaSession() {
    ...
    navigator.mediaSession.metadata = new MediaMetadata({
      title: l === 'ar' ? 'قذائف الحق' : l === 'fr' ? 'Missiles de Verite' : 'Missiles of Truth',
  ...
  // ═══ SETTINGS ═══
  function onSpeedChange(val) { STATE.speed = parseFloat(val); const label = document.getElementById('narratorSpeedLabel'); if (label) label.textContent = STATE.speed + 'x'; localStorage.setItem('qh-narrator-speed', STATE.speed); }
  function onPitchChange(val) { STATE.pitch = parseFloat(val); const label = document.getElementById('narratorPitchLabel'); if (label) label.textContent = STATE.pitch.toFixed(1); localStorage.setItem('qh-narrator-pitch', STATE.pitch); }
  ...
  function onKaraokeToggle(checked) { STATE.karaokeEnabled = checked; localStorage.setItem('qh-narrator-karaoke', checked); }
  function onAutoScrollToggle(checked) { STATE.autoScroll = checked; localStorage.setItem('qh-narrator-autoscroll', checked); }
  function onDuoToggle(checked) { STATE.duoReading = checked; localStorage.setItem('qh-narrator-duo', checked); }
  ...
  function loadSettings() {
    STATE.speed = parseFloat(localStorage.getItem('qh-narrator-speed')) || 1;
    STATE.pitch = parseFloat(localStorage.getItem('qh-narrator-pitch')) || 1;
    STATE.karaokeEnabled = localStorage.getItem('qh-narrator-karaoke') !== 'false';
    STATE.autoScroll = localStorage.getItem('qh-narrator-autoscroll') !== 'false';
    STATE.duoReading = localStorage.getItem('qh-narrator-duo') === 'true';
  }
  ...
  function injectSpeakButtons() {
    ...
    var selectors = [
      ...
      '.truth-card', '.myth-flip-card', '.daily-card'
    ];
  ...
  if (origRenderTruths) { window.renderTruths = function() { origRenderTruths(); setTimeout(injectSpeakButtons, 100); }; }
  var origRenderMyths = window.renderMyths;
  if (origRenderMyths) { window.renderMyths = function() { origRenderMyths(); setTimeout(injectSpeakButtons, 100); }; }
